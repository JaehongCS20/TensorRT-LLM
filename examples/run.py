...
def parse_arguments(args=None):
    ...
    parser.add_argument('--profile_iter', type=int, default=10)
...
def print_output(tokenizer,
    ...
    if output_csv is None and output_npy is None:
        for i in range(batch_size * num_return_sequences):
            ...
            if seq_idx == 0:
                print(f'Input [Text {batch_idx} Length {input_lengths[batch_idx]}]: \"{input_text}\"')
            ...
            for beam in range(num_beams):
                ...
                output_text = ''
                for i, tok in enumerate(outputs):
                    if i == len(outputs) - 1:
                        output_text += tokenizer.decode(tok)
                    else:
                        output_text += tokenizer.decode(tok) + ' '

                index_str = (f'Text {batch_idx} Seq {seq_idx} Beam {beam}'
                             if num_return_sequences > 1 else
                             f'Text {batch_idx} Length {len(outputs)}')
...
def main(args):
    ...
    engine_loaded = False
    for i in range(1, args.max_input_length+1):
        for j in range(1, args.max_output_len+1):
            new_input = ""
            for k in range(i):
                if k == i - 1:
                    new_input += 'The'
                    ...
                    new_input += 'The '
            args.input_text = [new_input]
            args.max_output_len = j
            ...
            if args.run_profiling:
                ...
                for _ in range(args.profile_iter):
                    with torch.no_grad():
                        outputs = runner.generate(
                            batch_input_ids,
                            max_new_tokens=args.max_output_len,
                ...
                print(
                    f"batch_size: {len(batch_input_ids)}, avg latency of {args.profile_iter} iterations: : {tensorrt_llm.profiler.elapsed_time_in_sec('tmp') / args.profile_iter} sec"
                )
